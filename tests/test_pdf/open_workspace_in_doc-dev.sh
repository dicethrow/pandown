...
lxc shell doc-dev -- sh -c "chown -R ubuntu:ubuntu ${remote_dir}" # although <my main user account> and ubuntu both have uid=1000, <my other user account> is 1001 so needs this to make copied files have the right permissions. todo - fix this in lxdev?
